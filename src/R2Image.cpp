...
SobelX(void)
{
	R2Image temp_img(width, height);
  double kernel[9] = {-1, 0, 1, -2, 0, 2, -1, 0, 1};
  applyKernel(&temp_img, kernel, 3, 3);

  //turn grayscale
	for( int i = 0; i < width; i ++){
		for (int j = 0; j < height; j++){
			double lum = temp_img.Pixel(i,j).Luminance() + 0.5;
			SetPixel(i,j, R2Pixel(lum, lum, lum, 1.0));
			Pixel(i,j).Clamp();  
		}
	}
}
...
GrayScale()
{
  for (int i = 0; i < width; i++) {
    for (int j = 0;  j < height; j++) {
      double lum = Pixel(i,j).Luminance();
      SetPixel(i,j, R2Pixel(lum, lum, lum, 1.0));
    }
  }
}
...
void R2Image::
applyKernel(R2Image * img, double kernel[], int kernel_width, int kernel_height)
{
  for(int i = 0; i < width; i++){
    for(int j = 0; j < height; j++){

      R2Pixel tempPixel;
      //cycling through kernel
      for(int l = 0; l < kernel_height; l++){
        for(int k = 0; k < kernel_width; k++){
          int pixelX = i + k - kernel_width/2;
          int pixelY = j + l - kernel_height/2;
          if(pixelX >= width){
            pixelX = width - 1;
          }
          else if(pixelX < 0){
          ...
          if(pixelY >= height){
           ...
           // std::cout << kernel[l * kernel_width + k] << "  " << k << 
           // "  " << l << "  " << l * kernel_width + k << std::endl;
          tempPixel += kernel[l * kernel_width + k] * Pixel(pixelX, pixelY);
        }
      }
      img -> SetPixel(i,j, tempPixel);
    }
  }
}
...
Blur(double sigma)
{
  ...
  int radius = sigma * 3;
  double *kernel;
  kernel = (double *) malloc((radius * 2 + 1) * sizeof(double));

  double sum = 0;

  for(int i = 0; i < radius + 1; i ++){
    double temp_val = exp(-(pow(i, 2)/(2 * pow(sigma, 2))));
    kernel[radius + i] = temp_val;
    kernel[radius - i] = temp_val;
    if(i == 0)
    {
      sum += temp_val;
  ...
  for(int i = 0; i < radius * 2 + 1; i ++){
    ...
    //std::cout << "kernel: " << i + 1 << "  " << kernel[i] << std:: endl;
  }

  R2Image temp_img(width, height);

  //first horizontal pass of kernel
  applyKernel(&temp_img, kernel, sigma * 6 + 1, 1);

  for( int i = 0; i < width; i ++){
    for (int j = 0; j < height; j++){
      SetPixel(i,j, temp_img.Pixel(i,j));
    }
  }

  //second vertical pass of kernel
  applyKernel(&temp_img, kernel, 1, sigma * 6 + 1);
...
{
  R2Image temp_img(width, height);
  double kernel[9] = {-1.0/9.0, -1.0/9.0, -1.0/9.0, -1.0/9.0,
